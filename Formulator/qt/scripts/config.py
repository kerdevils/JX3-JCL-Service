...
def config_script(
        ...
        equipments_widget: EquipmentsWidget, consumable_widget: ConsumablesWidget,
        bonus_widget: BonusesWidget, manual_widget: ManualAttrWidget = None):
    ...
    def load_manual_attrs(config):
        if not config or not manual_widget:
            return
        manual_widget.enable_checkbox.setChecked(config.get("enabled", False))
        for key, value in config.get("attrs", {}).items():
            if key in manual_widget.spin_boxes:
                manual_widget.spin_boxes[key].spin_box.setValue(value)
    ...
    def load_config():
        ...
        if category == "全部":
            ...
            load_manual_attrs(config.get("manual_attrs"))
    ...
    def save_manual_attrs(config):
        if not manual_widget:
            return
        config["enabled"] = manual_widget.enable_checkbox.isChecked()
        config["attrs"] = {}
        for key, spin in manual_widget.spin_boxes.items():
            value = spin.spin_box.value()
            if value != 0:
    ...
    def save_config():
        ...
        save_manual_attrs(CONFIG[name][config_name]['manual_attrs'])
