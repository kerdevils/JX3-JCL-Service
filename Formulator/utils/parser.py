...
class Parser(BaseParser):
            ...
            for buff_id, buff_levels in buff_ids.items():
                ...
                for buff_level, end_frame in buff_levels.items():
                    if end_frame < self.current_frame:
                        ...
                        if not self.buff_stacks[caster_id][buff_id]:
                            self.buff_stacks[caster_id].pop(buff_id, None)
    ...
    def parse_buff(self, row):
        ...
        # JCL supplies the authoritative expiration frame for direct buff events.
        end_frame = int(detail[6])
        buff_id, buff_stack, buff_level = detail[4], detail[5], detail[8]
        ...
        if buff_stack:
            ...
            if end_frame > self.current_frame:
                self.buff_intervals[caster_id][buff_id][buff_level] = end_frame
            buff.begin(self)
        else:
            self.buff_intervals[caster_id][buff_id].pop(buff_level, None)
            if not self.buff_intervals[caster_id][buff_id]:
                self.buff_intervals[caster_id].pop(buff_id, None)
            if buff.continuous:
    ...
    def filter_buff(buff, damage):
        is_key_skill = getattr(damage, "key_skill", False)
        if not (damage.damage_call or is_key_skill):
            return False
        if buff.recipes:
            return True
        if buff.attributes:
            if (
                all("attack_power" in attr for attr in buff.attributes)
                and not (damage.attack_power_call or is_key_skill)
            ):
                return False
            elif (
                all("surplus" in attr for attr in buff.attributes)
                and not (damage.surplus_call or is_key_skill)
            ):
                return False
            return True
